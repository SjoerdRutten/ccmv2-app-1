        ...
        @elseif (Arr::get($filter, 'columnType') === 'integer')
            ...
            @if (Arr::get($filter, 'operator'))
                @if (Arr::get($filter, 'operator') === 'between')
                    <x-ccm::forms.input type="number" step="1" name="filter.from" wire:model.blur="filter.from"/>
                    <x-ccm::forms.input type="number" step="1" name="filter.to" wire:model.blur="filter.to"/>
                @else
                    <x-ccm::forms.input type="number" step="1" name="filter.value" wire:model.blur="filter.value"/>
                @endif
            @endif
